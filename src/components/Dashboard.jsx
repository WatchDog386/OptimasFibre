...
const downloadReceiptPdf = async (receiptId, API_BASE_URL, token, showNotification) => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/receipts/${receiptId}/pdf`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      showNotification(`❌ Failed to download receipt PDF (${response.status})`, 'error');
      return { success: false };
    }

    const blob = await response.blob();
    if (blob.type !== 'application/pdf') {
      showNotification('❌ Server returned non-PDF response for receipt.', 'error');
      return { success: false };
    }
    ...
    showNotification('❌ Network error downloading receipt PDF.', 'error');
    return { success: false };
  }
};

// ✅ UPDATED: Send flow = download PDF then redirect to webmail
const sendInvoiceViaEmail = async (invoice, API_BASE_URL, token, showNotification) => {
  try {
    console.log(`📧 Preparing invoice ${invoice._id} for webmail send...`);
    ...
    const pdfResult = await downloadInvoicePdf(invoice._id, API_BASE_URL, token, showNotification);
    ...
    const draft = [
      ...
      'Subject: Invoice from OPTIMAS FIBER',
      '',
    ...
    try {
      ...
      showNotification('📋 Invoice email draft copied. Paste it in webmail compose.', 'success');
    } catch {
      showNotification('ℹ️ PDF downloaded. Compose invoice email manually in webmail.', 'info');
    }
...
const sendReceiptViaEmail = async (receipt, API_BASE_URL, token, showNotification) => {
  try {
    console.log(`📧 Preparing receipt ${receipt._id} for webmail send...`);
    ...
    const pdfResult = await downloadReceiptPdf(receipt._id, API_BASE_URL, token, showNotification);
    if (!pdfResult.success) {
      return { success: false };
    }
    ...
    const draft = [
      ...
      'Subject: Receipt from OPTIMAS FIBER',
      '',
      `Dear ${receipt.customerName || 'Valued Customer'},`,
      'Please find attached your receipt PDF.',
      '',
      'Regards,',
      'OPTIMAS FIBER'
    ].join('\n');
    ...
    try {
      await navigator.clipboard.writeText(draft);
      showNotification('📋 Receipt email draft copied. Paste it in webmail compose.', 'success');
    } catch {
...
const Dashboard = () => {
  ...
  const renderContent = () => {
    ...
    if (error && activeTab === 'dashboard') {
      return (
        <div className={`p-6 mb-6 rounded-xl ${darkMode ? 'bg-red-900/20 border border-red-800/30' : 'bg-red-50 border border-red-200'}`}>
    ...
    <div className={`flex h-screen ${themeClasses.background} ${themeClasses.text} transition-colors duration-300`}>
      ...
      {loadingPopup.show && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div ref={loadingPopupRef} className={`rounded-xl shadow-2xl p-8 max-w-md w-full mx-4 ${themeClasses.card} flex flex-col items-center`}>
      ...
      <div className={`${themeClasses.card} w-64 flex-shrink-0 shadow-lg fixed md:relative z-30 h-full transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 border-r ${darkMode ? 'border-gray-700' : 'border-[#00356B]/10'}`}>
      ...
      <div className="flex-1 overflow-auto">
        <header className={`${themeClasses.card} shadow-md p-4 md:p-6 flex items-center justify-between border-b ${darkMode ? 'border-gray-700' : 'border-[#00356B]/10'} sticky top-0 z-10`}>
...
const CalendarPlaceholder = ({ darkMode, themeClasses }) => {
  ...
  return (
    <div className={`${themeClasses.card} p-4 rounded-xl shadow-sm border h-full`}>
...
const ReceiptStatusPieChart = ({ receipts, darkMode, themeClasses }) => {
  ...
  return (
    <div className={`${themeClasses.card} p-4 rounded-xl shadow-sm border h-full flex flex-col`}>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-1 h-80">
          <div className={`${themeClasses.card} p-5 rounded-xl shadow-md border h-full`}>
...
const StatCard = ({ title, value, change, icon, color, darkMode }) => {
  ...
  return (
    <div className={`${colorClasses[color]} p-6 rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1`}>
...
const RecentList = ({ title, items, viewAllLink, darkMode, themeClasses, onEdit, onDelete, type, API_BASE_URL, showNotification }) => {
  const handleDownloadPDF = async (item) => {
    ...
    if (type === 'receipts') {
      await downloadReceiptPdf(item._id, API_BASE_URL, token, showNotification);
    } else {
  ...
  return (
    <div className={`${themeClasses.card} p-6 rounded-xl shadow-md border transition-all duration-300 hover:shadow-lg`}>
...
const ContentManager = ({
  ...
  return (
    <div>
      ...
      <div className={`${themeClasses.card} p-6 md:p-8 rounded-xl shadow-lg border mb-8`}>
        <div className="space-y-6">
          <InputGroup
            label="Title"
            name="title"
      ...
      <div className={`${themeClasses.card} p-6 rounded-xl shadow-lg border overflow-x-auto`}>
...
const SettingsPanel = ({ settingsData, handleSettingsChange, handleCompanyInfoChange, saveSettings, darkMode, themeClasses }) => (
  <div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className={`${themeClasses.card} p-6 md:p-8 rounded-xl shadow-lg border`}>
